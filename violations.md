...
The screenshots shown are generated visualisations of individual conversations, combining information such as turn-by-turn dialogue, outcomes, annotations, and rule violation predictions. You can create these visualisations using `dev/visualise_conv.py`. For batch visualisation or analysis in this project, see the workflow demonstrated in `riverline_takehome/dev/run_evals.ipynb`.
